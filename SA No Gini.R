...
mydata <- read_xlsx("SA Diff.xlsx")
summary(mydata)
linearreg <- lm(CO2 ~ GDP + Urb + FDI, data=mydata)
summary(linearreg)
...
#Whites Test
bptest(linearreg, ~ + GDP*Urb + GDP*FDI + Urb*FDI + I(GDP^2) + I(Urb^2) + I(FDI^2), data=mydata)
...
bgtest(CO2 ~ GDP + Urb + FDI, order=4, data=mydata)

#durbins h test
n <- nrow(mydata)
mydata$CO2_l1 <- c(NA, mydata$CO2[-n])
reg_lag <- lm(CO2 ~ GDP + Urb + FDI + CO2_l1, data=mydata)
