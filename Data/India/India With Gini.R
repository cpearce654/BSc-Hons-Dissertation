...
mydata <- read_xlsx("India Diff Gini.xlsx")
summary(mydata)
linearreg <- lm(CO2 ~ GDP + Urb + FDI + Gini, data=mydata)
summary(linearreg)
...
Urb<-ts(mydata$Urb, start=1961, frequency=1)
...
#Whites Test
bptest(linearreg, ~ + GDP*Urb + GDP*FDI + Urb*FDI + Gini*GDP + Gini*Urb + Gini*FDI + I(GDP^2) + I(Urb^2) + I(FDI^2) + I(Gini^2), data=mydata)
...
bgtest(CO2 ~ GDP + Urb + FDI + Gini, order=4, data=mydata)

#durbins h test
n <- nrow(mydata)
mydata$CO2_l1 <- c(NA, mydata$CO2[-n])
reg_lag <- lm(CO2 ~ GDP + Urb + FDI + Gini + CO2_l1, data=mydata)
...
reg <- lm(CO2 ~ GDP + Urb + FDI, data = mydata)
...
# Null hypothesis: Urb does not Granger cause CO2
grangertest(CO2 ~ Urb, data = mydata, order = 2)
